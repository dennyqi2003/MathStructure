...
In this task, you are part of larger system. A Python program is recursively walking a JSON tree and will call you for **every single node** it encounters. Your job is to provide the correct line of Lean 4 code for that *one node*, given its context.

**CRITICAL RULES:**

* You **MUST NOT** try to translate the entire JSON tree. Only translate the single `current_node` provided.
* You **MUST NOT** try to solve the proof. Any proof block (like for a `Have` or `Obtain` node) must end in `by sorry`.
* You **MUST** pay close attention to the context, this is how you decide which Lean 4 syntax to use.
* Your output **MUST** be a the lean code corresponding to that node.
...
Your entire output **MUST** be a single, valid JSON object with one key:
  ...
  * **`lean_code`**: A string containing the translated Lean 4 code snippet for the *single* node provided.

-----

## 4\. Translation Manual & Examples

Use this guide to translate each `current_node` based on its `translation_context`.

### 'Show' Node

  * **If `translation_context` is `"top_level"`:**
      ...
      * **Input:**
        ```json
        {
          "current_node": {
            "type": "Show",
            "proposition": ["My Theorem: $\\forall x, P(x)$"],
            "method": null
          },
          "translation_context": "top_level"
        }
        ```
      * **Output:**
        ```json
        {"lean_code": "theorem My_Theorem : ∀ x, P(x) := by"}
        ```

  * **If `translation_context` is `"tactic_mode"`:**
      ...
      * **Input:**
        ```json
        {
          ...
          "translation_context": "tactic_mode"
        }
        ```
      * **Output:**
        ```json
        {"lean_code": "show A ⊆ B by"}
        ```
...
### 'Assume' Node
  ...
  * **If `translation_context` is `"tactic_mode"`:** (This node only appears in tactic mode)
      * **Input:**
        ```json
        {
          "current_node": {
      ...
      * **Output:**
        ```json
        {"lean_code": "intro h_n_is_even : IsEven n"}
        ```
...
### 'Have' Node
  ...
  * **If `translation_context` is `"tactic_mode"`:** (This node only appears in tactic mode)
      ...
      * **Output:**
        ```json
        {"lean_code": "have h_x_subset_B : x ⊆ B := by sorry"}
        ```
...
### 'Obtain' Node
  ...
  * **If `translation_context` is `"tactic_mode"`:** (This node only appears in tactic mode)
      ...
      * **Output:**
        ```json
        {"lean_code": "obtain ⟨k, hk_int, hn_eq_2k⟩ : ∃ k : Int, n = 2 * k := by sorry"}
        ```
...
### 'Define' Node
  ...
  * **If `translation_context` is `"tactic_mode"`:** (This node usually appears in tactic mode)
      * **Input:**
        ```json
        {
          "current_node": {
...
### 'Hint' Node
  ...
  * **This node translates to a Lean comment in all contexts.**
      * **Input:**
        ```json
        {
          "current_node": {
            "type": "Hint",
            "text": "This is a clever idea."
          },
          "translation_context": "tactic_mode"
        }
        ```
      * **Output:**
        ```json
        {"lean_code": "-- This is a clever idea."}
        ```
...
### 'SufficesToProve' Node
  ...
  * **If `translation_context` is `"tactic_mode"`:**
      ...
      * **Output:**
        ```json
        {"lean_code": "suffices h_suffices : x ∈ A by sorry"}
        ```

-----

### 'LogicChain' & 'CalculationChain' Nodes

  * These nodes are complex. **Translate only the *initial expression***. The Orchestrator will handle iterating through the steps and calling you again for each step (which will be a `Have` node).
      * **Input:**
        ```json
        {
          "current_node": {
...
### 'Find' Node
  ...
  * This node is a top-level goal, similar to 'Show'.
  * **If `translation_context` is `"top_level"`:**
      * **Input:**
        ```json
        {
      ...
      * **Output:**
        ```json
        {"lean_code": "def solution : {x // x^2 - 3*x + 4 = 0} := by sorry"}
        ```